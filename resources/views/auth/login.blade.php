...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-brand-ink text-brand-ink antialiased">
        <main class="grid min-h-screen lg:grid-cols-[1.05fr_0.95fr]">
            <section class="relative hidden overflow-hidden bg-brand-ink text-white lg:block">
                ...
                <div class="absolute inset-0 bg-brand-ink/65"></div>
                <div class="absolute left-0 top-0 h-1 w-full bg-brand-orange"></div>
                <div class="relative flex h-full flex-col justify-between p-10">
                    <a href="{{ route('home') }}" class="flex items-center gap-3">
                        <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-red text-sm font-semibold text-white">GH</span>
                        <span>
                            <span class="block text-xl font-semibold">GastroHub</span>
                            <span class="block text-xs font-medium uppercase tracking-[0.16em] text-brand-orange">Restaurante digital</span>
                        </span>
                    </a>

                    <div class="max-w-xl">
                        <p class="gh-page-kicker text-brand-orange">Panel interno</p>
            ...
            <section class="flex min-h-screen items-center justify-center bg-brand-stone px-4 py-10">
                <div class="w-full max-w-md">
                    <div class="mb-8 text-center lg:hidden">
                        <a href="{{ route('home') }}" class="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-red text-sm font-semibold text-white">GH</a>
                    ...
                    <div class="gh-panel p-6 sm:p-8">
                        ...
                        <form method="POST" action="{{ route('login.store') }}" class="mt-6 space-y-5">
                            ...
                            <label class="flex min-h-11 items-center gap-3 rounded-xl border border-zinc-200 bg-zinc-50 px-3 text-sm text-zinc-700">
                                <input type="checkbox" name="remember" value="1" class="rounded border-zinc-300 text-brand-red">
                                Recordar sesion
                            </label>
