...
@section('content')
    @php
        ...
        $secondaryColor = $restaurant?->safeSecondaryColor() ?? '#000000';
    @endphp
    ...
    <section class="relative overflow-hidden text-white" style="--brand-primary: {{ $primaryColor }}; --brand-secondary: {{ $secondaryColor }}; background-color: var(--brand-secondary);">
        <img src="{{ $restaurant?->coverImageUrl() ?? asset('images/restaurant-hero.png') }}" alt="Portada de {{ $restaurant?->name ?? 'GastroHub' }}" class="absolute inset-0 h-full w-full object-cover opacity-65">
        <div class="absolute inset-0 bg-gradient-to-r from-brand-ink/90 via-brand-ink/68 to-brand-ink/20"></div>
        <div class="absolute left-0 top-0 h-1 w-full bg-brand-orange"></div>
        <div class="relative mx-auto grid min-h-[calc(100vh-4.25rem)] max-w-6xl content-center gap-8 px-4 py-12 lg:min-h-[38rem]">
            <div class="max-w-3xl gh-reveal">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <span class="rounded-full bg-brand-red/90 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-white ring-1 ring-white/15">{{ $restaurant->tagline ?? 'Restaurante digital' }}</span>
                ...
                <div class="mt-7 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ route('menu') }}" class="gh-btn min-h-12 rounded-2xl bg-brand-red px-6 text-white hover:bg-brand-red-dark">Ver menu</a>
                    <a href="#contacto" class="gh-btn rounded-2xl bg-brand-orange px-6 text-brand-ink hover:bg-brand-orange-dark">Ubicacion y contacto</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <section id="contacto" class="mx-auto max-w-6xl px-4 py-10 sm:py-14">
        ...
        <div class="mt-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="gh-page-kicker">Favoritos</p>
        ...
        <div class="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($featuredProducts as $product)
                <article class="group overflow-hidden rounded-2xl border border-brand-brown/15 bg-white shadow-sm shadow-brand-ink/[0.04] transition duration-200 hover:-translate-y-1 hover:border-brand-orange/60 hover:shadow-xl hover:shadow-brand-ink/[0.08]">
                    <div class="relative overflow-hidden">
                        ...
                        <span class="absolute left-3 top-3 rounded-full bg-brand-orange px-3 py-1 text-xs font-semibold text-brand-ink shadow-sm">{{ $product->category->name }}</span>
        ...
        <section class="mt-12 overflow-hidden rounded-3xl bg-brand-ink p-6 text-white shadow-xl shadow-brand-ink/10 sm:p-8">
            <div class="grid gap-6 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-orange">Listo para ordenar</p>
                ...
                <div class="flex flex-col gap-3 sm:flex-row md:flex-col">
                    <a href="{{ route('menu') }}" class="gh-btn rounded-2xl bg-brand-red text-white hover:bg-brand-red-dark">Explorar menu</a>
