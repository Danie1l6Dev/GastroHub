        ...
        <header class="sticky top-0 z-40 border-b border-white/70 bg-white/85 shadow-sm shadow-zinc-950/[0.03] backdrop-blur-xl">
            @if ($isQrLayout)
                <div class="mx-auto flex max-w-6xl items-center justify-center px-4 py-3">
                    <a href="{{ route('home') }}" class="inline-flex min-h-12 max-w-full items-center justify-center gap-3 rounded-2xl px-3 transition hover:bg-zinc-100" aria-label="Ir al inicio del restaurante">
                        @if ($restaurant?->logoUrl())
                            <img src="{{ $restaurant->logoUrl() }}" alt="" class="h-11 w-11 shrink-0 rounded-2xl object-cover">
                        @else
                            <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-zinc-950 text-sm font-semibold text-white">
                                {{ mb_substr($restaurant->name ?? 'G', 0, 1) }}
                            </span>
                        @endif
                        <span class="max-w-[14rem] truncate text-base font-semibold tracking-tight text-zinc-950 sm:max-w-md">
